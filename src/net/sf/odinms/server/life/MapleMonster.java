...
public class MapleMonster extends AbstractLoadedMapleLife {
	...
	public MapleCharacter killBy(MapleCharacter killer) {
		...
		if (toSpawn != null) {
			...
			if (toSpawn.contains(9300216) && reviveMap.getId() > 925020020 && reviveMap.getId() < 926000000) {
	...
	public boolean applyStatus (MapleCharacter from, final MonsterStatusEffect status, boolean poison, long duration, boolean venom) {
		...
		if (poison) {
			...
			if (from.getJob() == MapleJob.NIGHTLORD || from.getJob() == MapleJob.SHADOWER || from.getJob().isA(MapleJob.NIGHTWALKER1)) {
				int poisonLevel = 0;
				int matk = 0;
				...
				} else if(from.getJob().isA(MapleJob.NIGHTWALKER1)) {
