...
public class MapleTrade {
	...
	public void complete2() {
		...
		for (IItem item : exchangeItems) {
			if ((item.getFlag() & ItemFlag.KARMA.getValue()) == ItemFlag.KARMA.getValue())
				item.setFlag((byte)(item.getFlag() ^ ItemFlag.KARMA.getValue())); //items with scissors of karma used on them are reset once traded
...
//	}
	...
	public static void visitTrade(MapleCharacter c1, MapleCharacter c2) {
		if (c1.getTrade() != null && c1.getTrade().getPartner() == c2.getTrade() &&
			...
			c2.getMap() == c1.getMap()) { //checking to see if they are both on the same map
