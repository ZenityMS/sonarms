...
public class MapleCharacterUtil {
    ...
    private static Pattern petPattern = Pattern.compile("[a-zA-Z0-9_-]{4,12}");
	
    public static boolean canCreateChar(String name, int world) {
        if (name.length() < 3 || name.length() > 12 || !namePattern.matcher(name).matches() || MapleCharacter.getIdByName(name, world) != -1) {
            return false;
        }
        return true;
    }
