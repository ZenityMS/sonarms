...
public class NoticeCommand implements Command {
	...
	public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception, IllegalCommandSyntaxException {
	    if(splitted[0].equals("/notice")) {
			...
			MaplePacket packet;
			if (type != 7)
				packet = MaplePacketCreator.serverNotice(type, prefix +
						StringUtil.joinStringFrom(splitted, joinmod));
			else
				packet = MaplePacketCreator.yellowNotice(prefix +
						StringUtil.joinStringFrom(splitted, joinmod));
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] {
			new CommandDefinition("/notice", "[mcw] [n/p/l/nv/v/b/y] message", "", 1),
			new CommandDefinition("/say", "message", "send a message with your name as the prefix", 1),
		};
	}
