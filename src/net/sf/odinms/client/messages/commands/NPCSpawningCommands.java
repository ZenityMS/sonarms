...
public class NPCSpawningCommands implements Command {
	...
	public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception,
		...
		if (splitted[0].equals("/npc")) {
			...
			for (MapleMapObject npcmo : npcs) {
				...
				if (npc.isCustom()) {
					player.getMap().broadcastMessage(MaplePacketCreator.removeNPC(npcmo.getObjectId())); //c00l
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] {
			new CommandDefinition("/npc", "npcid", "Spawns the npc with the given id at the player position", 3),
			new CommandDefinition("/removenpcs", "", "Removes all custom spawned npcs from the map - requires reentering the map", 3),
			new CommandDefinition("/mynpcpos", "", "Gets the info for making an npc", 3),
		};
	}
