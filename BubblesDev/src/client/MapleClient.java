...
public class MapleClient {
    ...
    public int login(String login, String pwd, boolean ipMacBanned) {
        ...
        Connection con = DatabaseConnection.getConnection();
        try {
            PreparedStatement ps = con.prepareStatement("SELECT id, password, salt, banned, gm, pin FROM accounts WHERE name = ?");
    ...
    public boolean deleteCharacter(int cid) {
        ...
        try {
            ...
            rs.close();
            ps = con.prepareStatement("DELETE characters, famelog, inventoryitems, keymap, savedlocations, skillmacros, skills, wishlist, eventstats FROM characters, famelog, inventoryitems, keymap, savedlocations, skillmacros, skills, wishlist, eventstats WHERE characters.id = ? AND famelog.characterid = ? AND inventoryitems.characterid = ? AND keymap.characterid = ? AND savedlocations.characterid = ? AND skillmacros.characterid = ? AND skills.characterid = ? AND wishlist.charid = ? AND eventstats.characterid = ?");
            for (byte i = 1; i < 10; i++)
                ps.setInt(i, cid);
            ps.executeUpdate();
            ps.close();
