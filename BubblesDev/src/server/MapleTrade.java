    ...
    private void complete2() {
        ...
        for (IItem item : exchangeItems) {
            if ((item.getFlag() & InventoryConstants.Items.Flags.KARMA) == InventoryConstants.Items.Flags.KARMA)
                item.setFlag((byte) (item.getFlag() ^ InventoryConstants.Items.Flags.KARMA)); //items with scissors of karma used on them are reset once traded
