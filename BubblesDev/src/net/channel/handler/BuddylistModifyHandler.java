...
public final class BuddylistModifyHandler extends AbstractMaplePacketHandler {
    ...
    private CharacterIdNameBuddyCapacity getCharacterIdAndNameFromDatabase(String name) throws SQLException {
        PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT id, name, buddyCapacity FROM characters WHERE name LIKE ?");
        ps.setString(1, name);
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == 1) { // add
            ...
            if (ble != null && !ble.isVisible() && group.equals(ble.getGroup()))
                c.getPlayer().dropMessage(1, "You already have \"" + ble.getName() + "\" on your Buddylist");
            else if (buddylist.isFull() && ble == null)
                c.getPlayer().dropMessage(1, "Your buddylist is already full");
            else if (ble == null)
                try {
                    CharacterIdNameBuddyCapacity charWithId = null;
                    int channel;
                    MapleCharacter otherChar = c.getChannelServer().getPlayerStorage().getCharacterByName(addName);
                    if (otherChar.gmLevel() > c.getPlayer().gmLevel()) {
                    ...
                    if (charWithId != null) {
                        ...
                        if (buddyAddResult != BuddyAddResult.OK)
                            c.getSession().write(MaplePacketCreator.sendBuddyError(buddyAddResult));
                        else {
...
//                                PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("INSERT INTO buddies (characterid, `buddyid`, `pending`) VALUES (?, ?, 1)");
//                                ps.setInt(1, charWithId.getId());
//                                ps.setInt(2, player.getId());
//                                ps.executeUpdate();
//                                ps.close();
                        ...
                        c.getPlayer().message("A character called \"" + addName + "\" does not exist");
                } catch (Exception e) {
                }
            else {
