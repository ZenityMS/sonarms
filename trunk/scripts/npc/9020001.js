...
var prizeQtyEtc = Array(15, 15, 15, 15,15, 15, 8,15, 15, 15, 15,15, 15, 15,8, 5, 20);
...
function clear(stage, eim, cm) {
    ...
    var packetef = cm.showEffect("quest/party/clear");
    var packetsnd = cm.playSound("Party1/Clear");
    var packetglow = cm.environmentChange("gate",2);
    var map = eim.getMapInstance(cm.getPlayer().getMapId());
...
function failstage(eim, cm) {
    var packetef = cm.showEffect("quest/party/wrong_kor");
    var packetsnd = cm.playSound("Party1/Failed");
    var map = eim.getMapInstance(cm.getPlayer().getMapId());
    map.broadcastMessage(packetef);
    map.broadcastMessage(packetsnd);
}
                ...
                // check for people on ropes
                    ...
                    for (i = 0; i < objset.length; i++) {
                        for (j = 0; j < party.size(); j++) {
                            var present = currect[i].contains(party.get(j).getPlayer().getPosition());
                            if (present) {
                                objset[i] = objset[i] + 1;
