...
function action(mode, type, selection) {
	...
	if (cm.getChar().getMapId() != 800000000) {
		...
		if (status == 0) {
			cm.sendSimple ("How's the traveling? Are you enjoying it?\r\n#L0##bYes, I'm done with travelling. Can I go back to #m" + cm.getChar().getSavedLocation(SavedLocationType.WORLDTOUR).getMapId() + "#?#k#l\r\n#L1##bNo, I'd like to continue exploring this place.#k#l");
		} else if (status == 1) {
			if (selection == 0) {
			...
			if (map == null) {
				map = new SavedLocation(100000000, 0);
			}
			cm.warp(map.getMapId(), true);
			cm.dispose();
			}
		}
	}
