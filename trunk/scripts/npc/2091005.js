/*	
Based on Tratiors Edited by Arberb TO MAKE SEXIER :)
*/
...
function action(mode, type, selection) {
	if (cm.getPlayer().getMap().getId() == 925020001) {
		if (mode >= 0) {
			...
			if (selectedMenu == 0) {
				if (cm.getPlayer().getDojoFinished() == 0) {
					if (status == 0) {
				...
				if (status == 0) {
					...
					if(cm.getPlayer().getDojoPoints() >= points){
						cm.getPlayer().addDojoPoints(-points);
						cm.gainItem(belt , 1);
						cm.sendOk("You have received this belt and " + points +" have been taken away from you!");
					} else {
						cm.sendOk("You dont have enough points to receive this belt. ");
					}
					cm.dispose();
				}
				...
				if (status == 0) {
					...
					if (mode == 0) {
						...
						var total = cm.getPlayer().getDojoPoints();
						cm.getPlayer().addDojoPoints(-total);
				...
				if (status == 0) {
					...
					if (mode == 0) {
						...
						if(cm.getPlayer().getDojoCompleted() >= 100){
							cm.gainItem(1142033,1);
							cm.getPlayer().addDojoCompleted(-100);
