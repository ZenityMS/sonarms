    ...
    if (mode == -1)
        ...
        if (status == 1) {
            ...
            var type = Math.floor(Math.random()*22+1);
                 if (type < 10) cm.gainItem(junk[junkc], itemamount);
            else if (type < 13) cm.gainItem(scrolls[scrollsc], 1);
            else if (type < 22) cm.gainItem(eq[eqc], 1);
            else if (type < 23) cm.gainItem(unique[uniquec], 1);
            cm.dispose();
        }
    }
}