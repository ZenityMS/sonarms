...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            if (!(cm.getJobById()==100 || cm.getJobById()==320)) {
            ...
            if (cm.isQuestCompleted(100102)) {
                ...
                cm.dispose();
            } else if ((cm.getJobById()==310 || cm.getJobById()==320) && cm.getLevel() >= 70 &&  cm.getPlayer().getRemainingSp() <= (cm.getLevel() - 70) * 3) {
