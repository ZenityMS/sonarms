...
public class MapleStatEffect implements Serializable {
    ...
    private void applyBuffEffect(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary) {
        ...
        if (isBattleShip()) {
            ...
            localstatups = Collections.singletonList(new Pair<MapleBuffStat, Integer>(MapleBuffStat.MONSTER_RIDING, 0));
            if (applyto.getPirateShip() == null)
                applyto.setPirateShip(applyto.new PirateShip(applyto));
        }
        if (isPirateMorph())
    ...
    public boolean isInfusion() { //temp fix
//        return skill && (sourceid == 5121009 || sourceid == 5221010);
        return false;
    }
}