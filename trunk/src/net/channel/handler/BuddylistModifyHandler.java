...
public class BuddylistModifyHandler extends AbstractMaplePacketHandler {
    ...
    private void notifyRemoteChannel(MapleClient c, int remoteChannel, int otherCid, BuddyOperation operation) throws RemoteException {
        if (remoteChannel != -1) {
            ChannelWorldInterface channelInterface = c.getChannelServer().getWorldInterface().getChannelInterface(remoteChannel);
            channelInterface.buddyChanged(otherCid, c.getPlayer().getId(), c.getPlayer().getName(), c.getChannel(), operation);
        }
    }
}
