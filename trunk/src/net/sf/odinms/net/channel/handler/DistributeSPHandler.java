...
public class DistributeSPHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (skillid == 1000 || skillid == 1001 || skillid == 1002 || 
				skillid == 10001000 || skillid == 10001001 || skillid == 10001002) { // boo beginner skill
			int snailsLevel = player.getSkillLevel(SkillFactory.getSkill(player.getJob().isA(MapleJob.NOBLESSE) ? 10001000 : 1000));
			int recoveryLevel = player.getSkillLevel(SkillFactory.getSkill(player.getJob().isA(MapleJob.NOBLESSE) ? 10001001 : 1001));
			int nimbleFeetLevel = player.getSkillLevel(SkillFactory.getSkill(player.getJob().isA(MapleJob.NOBLESSE) ? 10001002 : 1002));
