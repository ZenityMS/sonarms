	...
	public void applyEffect(MapleCharacter player, MapleMonster monster, boolean skill) {
		...
		switch (skillId) {
			...
			case 129: // Banish?
				...
				break;
			case 140:
                if (!monster.isBuffed(MonsterStatus.MAGIC_IMMUNITY) && makeChanceResult())
                    monster.applyMonsterBuff(MonsterStatus.WEAPON_IMMUNITY, getX(), getSkillId(), getDuration(), this);
                break;
            case 141:
                if (!monster.isBuffed(MonsterStatus.WEAPON_IMMUNITY) && makeChanceResult())
                    monster.applyMonsterBuff(MonsterStatus.MAGIC_IMMUNITY, getX(), getSkillId(), getDuration(), this);
                break;
			case 200:
