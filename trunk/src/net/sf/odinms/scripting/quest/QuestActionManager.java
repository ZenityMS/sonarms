...
public class QuestActionManager extends NPCConversationManager {
	...
	public void forceCompleteQuest(boolean showQuestFinish) {
		forceCompleteQuest(quest, true);
	}

	public void forceCompleteQuest(int id) {
		forceCompleteQuest(id, true);	
	}
	
	public void forceCompleteQuest(int id, boolean showQuestFinish) {
		MapleQuest.getInstance(id).forceComplete(getPlayer(), getNpc());
		if (showQuestFinish) //finish quest effect
			getC().getSession().write(MaplePacketCreator.showSpecialEffect(9));
	}
}