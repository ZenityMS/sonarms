...
public class MapleInventoryManipulator {
    ...
    public static boolean addById(MapleClient c, int itemId, short quantity, String string) {
        return addById(c, itemId, quantity, string, -1);
    }
    ...
    public static boolean checkSpace(MapleClient c, int itemid, int quantity, String owner) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (slotMax > 0)
                ...
                numSlotsNeeded = 1;
                System.out.println("SUCK ERROR - FIX ME! - 0 slotMax");
            }
    ...
    public static void equip(MapleClient c, byte src, byte dst) {
        ...
        if (dst == -6) { // unequip the overall
            ...
            if (top != null && isOverall(source.getItemId())) {
                if (c.getPlayer().getInventory(MapleInventoryType.EQUIP).isFull(bottom != null && isOverall(source.getItemId()) ? 1 : 0)) {
                    c.getSession().write(MaplePacketCreator.getInventoryFull());
                    c.getSession().write(MaplePacketCreator.getShowInventoryFull());
                    return;
                }
    ...
    public static void drop(MapleClient c, MapleInventoryType type, byte src, short quantity) {
        ...
        if (!c.getPlayer().haveItem(itemId, quantity, true, true) || quantity < 0 || source == null || quantity == 0 && !InventoryConstants.isRechargable(itemId))
            return;
