        ...
        switch (type) {
            case EXP:
                ...
                if (status.getStatus() == MapleQuestStatus.Status.NOT_STARTED && status.getForfeited() > 0)
                    break;
                c.gainExp((int) (MapleDataTool.getInt(data) * ServerConstants.EXP_RATE * c.getExpMod()), true, true);
                break;
            case ITEM:
                try {
            ...
            case MESO:
                ...
                if (status.getStatus() == MapleQuestStatus.Status.NOT_STARTED && status.getForfeited() > 0)
                    break;
                c.gainMeso(MapleDataTool.getInt(data) * ServerConstants.MESO_RATE, true, false, true);
                break;
            case QUEST:
