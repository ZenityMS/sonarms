...
public final class TakeDamageHandler extends AbstractMaplePacketHandler {
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (damagefrom != -2) {
            ...
            if (player.getMap().getMonsterById(monsteridfrom) == null || attacker == null)
                return;
        ...
        if (damage == -1)
            fake = 4020002 + (int) (player.getJob().getId() / 10 - 40) * 100000;
        if (damage > 0 && !player.isHidden()) {
            ...
            if (damagefrom != -2) {
                ...
                if (jobid == 112 || jobid == 122 || jobid == 132) {
                    int id = jobid * 10000 + jobid == 112 ? 4 : 5;
                    achilles1 = SkillFactory.getSkill(id);
            ...
            if (player.getBuffedValue(MapleBuffStat.MAGIC_GUARD) != null && mpattack == 0) {
                ...
                if (player.getBuffedValue(MapleBuffStat.MONSTER_RIDING).intValue() == SkillConstants.Corsair.Battleship)
