                    ...
                    switch (APFrom) {
                        ...
                        case 2048: // HP
                            if (player.getHpApUsed() < 1 || player.getHpApUsed() == 10000)
                                return;
                            int loss = DistributeAPHandler.addHP(c);
                            if (c.getPlayer().getMaxHp() - loss < player.getLevel() * 2 + 148)
                                return;
                            DistributeAPHandler.addHP(c.getPlayer(), -DistributeAPHandler.addHP(c));
                            break;
                        case 8192: // MP
                            if (player.getHpApUsed() <= 0 || player.getMpApUsed() == 10000)
                                return;
                            int down = DistributeAPHandler.addMP(c);
                            if (player.getMaxMp() - down < player.getLevel() * 2 + 148)
                                return;
                            DistributeAPHandler.addMP(c.getPlayer(), -DistributeAPHandler.addMP(c));
                            break;
                        default:
...
//                    equip.setFlag((byte) InventoryConstants.Items.Flags.LOCK);
                ...
                switch (itemId / 1000 % 10) {
                    ...
                    case 5: // Maple TV
                        ...
                        if (!MapleTVEffect.getInstance().isActive()) {
